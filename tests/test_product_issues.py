...
class ProductIssueSwimlaneTests(ProductIssueFixture, unittest.TestCase):
    ...
    def test_the_lane_is_provisioned_before_the_occurrence_is_staged(self) -> None:
        ...
        self.assertEqual(
            self._lane_names(client)[int(self._created(client, "product:butler")["swimlane_id"])], "butler"
        )
    ...
    def test_nonpositive_create_proof_failures_keep_product_and_issue_pending(self) -> None:
        ...
        for entity_kind in ("product", "issue"):
            for failure in ("transport", "json_rpc", "malformed_list", "ambiguous_marker"):
                with (
                    self.subTest(entity_kind=entity_kind, failure=failure),
                    tempfile.TemporaryDirectory() as tmpdir,
                ):
                    self._nonpositive_create_proof_case(entity_kind, failure, Path(tmpdir))

    def _nonpositive_create_proof_case(self, entity_kind: str, failure: str, root: Path) -> None:
        """One entity kind and one proof failure, as its own call rather than a loop body.

        The fault injector closes over this call's arguments instead of the loop variables, so a
        case cannot be armed with the next case's failure mode or board client.
        """
        (root / "projects").mkdir()
        (root / "projects" / "secretary.yaml").write_text("id: secretary\n", encoding="utf-8")
        client = ProductBoard()
        ...
        for entity_kind in ("product", "issue"):
            for failure in ("transport", "json_rpc", "malformed_list", "semantic"):
                with (
                    ...
                    self._failing_swimlane_lookup_case(entity_kind, failure, Path(tmpdir))

    def _failing_swimlane_lookup_case(self, entity_kind: str, failure: str, root: Path) -> None:
        """One entity kind and one lane-lookup failure, as its own call rather than a loop body.

        The injector and the `create` helper close over this call's arguments, not the loop
        variables, so the retry after `fault_active` clears cannot read a later case's state.
        """
        (root / "projects").mkdir()
        (root / "projects" / "secretary.yaml").write_text("id: secretary\n", encoding="utf-8")
        client = ProductBoard()
        ...
        if entity_kind == "issue":
            store.create_product(
                ...
                description="",
                actor="po",
                request_id=f"seed-lane-{failure}",
            )
        original_call = client.call
        fault_active = True
    ...
    def test_quarantined_document_returns_through_adopt_and_retry(self) -> None:
        ...
        with mock.patch("secretary.product_issue_commands.board_client", return_value=client):
            output = io.StringIO()
...
class ProductIssueStoreTests(ProductIssueFixture, unittest.TestCase):
    ...
    def test_all_operations_restart_without_duplicate_backend_writes(self) -> None:
        ...
        self.assertEqual(
            len(
                client.comments[
                    int(next(task for task in client.tasks if task["reference"] == issue["ref"])["id"])
                ]
            ),
            2,
        )
        self.assertEqual(
            [event["kind"] for event in store.audit.events()],
            ["entity.created", "entity.created", "entity.updated", "issue.closed"],
        )
    ...
    def test_existing_product_retry_uses_its_staged_projects_before_the_registry(self) -> None:
        ...
        product = ProductIssueStore(
            self.client, data_dir=self.root / "data", instance=self.root
        ).create_product(
            product_id="secretary",
            projects=["secretary"],
            title="Secretary",
