...
pass with no bound reads everything and sets one; a pass with a bound asks the audit only for what
settled since; a pass that leaves an event eligible keeps the old bound so the event is read again.
Whether an event was already charged is its own request id's lookup, not a set built from the read.
"""
...
from secretary.dispatch.production import BUDGET_WINDOW_KEY, BUDGET_WINDOW_OVERLAP, _reconcile_sprint_budget
...
class BudgetWindowTests(unittest.TestCase):
    ...
    def test_a_pass_without_a_bound_reads_everything_and_sets_one(self) -> None:
        ...
        self.assertEqual(audit.reads, [{"reference": ""}])
    ...
    def test_an_unreadable_bound_reads_everything(self) -> None:
    ...
    def test_without_the_production_state_nothing_is_remembered(self) -> None:
        audit = _Audit([_hotfix("evt-1")])
