...
plus the public reason/actor and the refs stopped by a hard pause. `excluded_worker` names a
narrow initiator/operator exception: a worker workspace deliberately left running while every
other In-progress worker is stopped.
...
wrote. The only live reader is runtime/dispatch.py (steward/curator/retro dispatch), through
is_paused(). Kept in its own tiny module (state.py primitives only, no ops import) so
runtime/dispatch.py can read it without pulling in the board machinery — the same reason it
lazy-imports agents.pipeline.health rather than importing more of this package.
"""
...
def hard_pause_auto_resume_status(state: dict, *, now: datetime | None = None) -> dict:
    ...
    No caller left in this tree: the pipeline dispatcher that drove it is gone (secretary-1135) and
    the production dispatcher decides its own auto-resume in `secretary/dispatcher_pause.py`.
    """
