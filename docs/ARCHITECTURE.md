...
Канон голов попадает в установку одним способом: `secretary upgrade` генерирует
`heads/heads.yaml` и рядом пишет `heads/source.yaml` с каноническим файлом, чекаутом и ревизией, из
которых снапшот сделан. Канон — `heads/heads.toml` самой установки, если он есть; иначе портативный
дефолт продукта `triggered_agents/agents/pipeline/heads.toml`. Какие аккаунты, модели и цепочки
fallback существуют — конфигурация установки, а не код продукта. Живой тик читает только снапшот, поэтому правка
