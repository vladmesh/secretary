...
Единственный писатель checkpoint работает в конце тика диспетчера под `tick_lock`. Отдельная
модель конкуренции не нужна: тик уже сериализован. Writer стейджит только фиксированный pathspec
`state/`, никогда `git add -A`, чтобы не затронуть ручные незакоммиченные правки конфига.
...
- секрет-скан `state/` чист. `state/` уходит на remote, это единственное место возможной утечки
  секрета (вставленный токен в карточке или логе). Опора на `redact.py`.
