...
| A project runs without Orca: optional `orca_binding`, no Orca kind in reconcile or doctor | merged, live proof pending | secretary-1704 (7f092ae). Live proof: a `project add` / `reconcile apply` after the final upgrade. |
...
7. **The legacy `CODEX_HOME` rung** (`~/.config/orca/codex-runtime-home/home`) in
   ...
   legacy home.
8. **`orca_binding` and the `orca` records in `host-managed.json`.** Why: only `orca-legacy` heads
   read `orca_binding`, and reconcile and doctor already ignore the leftover `orca` records.
   Durable formats stay loadable: the loader keeps accepting and ignoring the key and the record
   until the instance drops them (a PO edit).
