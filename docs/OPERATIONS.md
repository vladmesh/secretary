...
- непустой `launch_intent` в записи — тик умер, не успев записать исход запуска. Ближайший тик
  ...
  заново. Руками тут делать нечего.

У принятой таким образом головы нет handle: тик, поднявший её, не дожил до записи. Живость такой
головы читается по pid-heartbeat, и в статусе терминала она отдаётся как `reason: pid`.
