...
`consumer: legacy-dispatcher`. Отдельно чистить его не надо: первый же `project gate` по проекту
перезаписывает draft без этого блока, в том числе когда проект уже enabled и gate только отдаёт
готовый passed-результат. До этого прогона `secretary doctor` будет ругаться на неизвестное поле в
draft.
...
Ту же телеметрию читает сигнал `pipeline_ticks` в `python3 -m triggered_agents steward scan`:
нездоровые тики с прошлого watermark. Дедуп идёт по монотонному счётчику `unhealthy_total`,
поэтому обычный здоровый тик между двумя прогонами стюарда не съедает аномалию, которую тот ещё
не посмотрел. Первый прогон без счётчика в watermark берёт текущее значение как baseline, а не
проигрывает заново всё, что сохранилось в кольце; baseline сохраняется сразу тем же прогоном
...
Счётчик имеет смысл только внутри одной истории телеметрии, поэтому рядом с ним диспетчер держит
`generation` — идентификатор истории, который выдаётся один раз и дальше не меняется. Watermark
стюарда хранит его вместе со счётчиком: как только generation другой (восстановление из бэкапа,
пересобранная установка, правка руками), стюард получает хит `pipeline-telemetry-reset` и
перечитывает всё кольцо заново. Одного счётчика для этого мало — новая история может встать ровно
на то число, которое watermark уже видел, и тогда её падения молча дедуплицировались бы. Счётчик,
уехавший назад, по-прежнему считается сбросом сам по себе: на установке, чей диспетчер ещё не
пишет `generation`, это единственный признак.
