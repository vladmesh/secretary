...
Теперь каждое ожидание закрывается по двум сигналам: терминал головы пропал из инвентаря orca,
либо ожидание превысило потолок. Первое срабатывание — один respawn той же головы в том же
воркспейсе, второе — карточка в Blocked с сигналом оператору. Ошибка самого запроса к orca за
смерть головы не считается: тогда работает только потолок.

- `SECRETARY_REVIEW_VERDICT_STALL_SECONDS` — потолок ожидания вердикта, дефолт 7200.
- `SECRETARY_WORKER_REPORT_STALL_SECONDS` — потолок ожидания отчёта воркера, дефолт 21600.
- `SECRETARY_WAIT_LIVENESS_GRACE_SECONDS` — сколько ждать после запуска, прежде чем верить ответу
  «терминала нет» (он появляется в инвентаре не мгновенно), дефолт 180.
