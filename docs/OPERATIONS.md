...
возраста карточки. У Codex TUI alternate screen может не обновлять `lastOutputAt`, поэтому для
профилей `codex_mode: tui` сигнал дополняется mtime rollout JSONL, привязанного к worktree. Это
проверяет только метаданные файла, не читает текст сессии. Потолки остаются fallback для старого
Orca без `lastOutputAt`. Первое превышение — один
