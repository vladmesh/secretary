...
- строка `pipeline` строится по телеметрии тиков production dispatcher —
  ...
  тиком. Тик, упавший с исключением (недоступная доска роняет первое же чтение задач), пишет
...
`data_dir` из instance (`SECRETARY_INSTANCE`, по умолчанию `/home/dev/secretary-instance`) — это
единственное, что packaged-юнит передаёт тиком `--instance`. `TA_PRODUCTION_STATE` переопределяет
