...
The memory runtime loads a local embedding model, and it is the appliance's dominant memory consumer: in
production it has reached roughly 1.9 GiB RSS at startup, and a target with 1.9 GiB of total RAM could not
finish a live index rebuild. The supported minimum has not been established; do not treat a 2 GiB profile
as proven.
...
comments of all cards in one batched JSON-RPC request. An export of a couple of hundred cards takes about
a second, so the tick stays inside its 60-second budget.
