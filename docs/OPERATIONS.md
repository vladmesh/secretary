...
Теперь на каждом тике ожидания диспетчер сверяет сохранённый handle активной попытки с
`orca terminal list`. Для ревьюера сверка использует также сохранённый `leafId`, потому что Orca
может выдать другой handle тому же pane. Отсутствующий или disconnected pane сразу запускает тот
...
`worker-runtime-unavailable` или `review-runtime-unavailable` и не трогает карточку.
...
Потолки также остаются fallback для старого Orca без `lastOutputAt`. Первое превышение — один
respawn той же головы в том же воркспейсе, второе — карточка в Blocked с сигналом оператору.
...
- `SECRETARY_REVIEW_VERDICT_STALL_SECONDS` — потолок ожидания вердикта и отсутствия вывода,
  дефолт 600 секунд.
- `SECRETARY_WORKER_REPORT_STALL_SECONDS` — потолок ожидания отчёта и отсутствия вывода,
  дефолт 1800 секунд.
