...
Роль фонового агента (curator, steward, retro) тоже маршрутизируется реестром: `automation.toml` в
продукте не называет профиль, голова берётся из `role_defaults.<роль>` и резолвится через
health.resolve_head так же, как голова воркера при клейме. Портативный дефолт держит перекрёстные
fallback-цепочки между семействами, поэтому установка с одной только Claude-подпиской и установка с
одной только OpenAI-подпиской поднимают все роли; установка, которой нужен claim-skip вместо
перехода в другое семейство, объявляет пустые цепочки в своём реестре.
