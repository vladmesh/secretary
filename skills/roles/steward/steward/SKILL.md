...
- **Идеи→Ready чужих карточек — не твой гейт.** Переход технически доступен роли, но продвижение
  ...
  ждут владельца. Свои срочные инфраструктурные задачи создавай сразу `create --column Ready` —
...
- **Дрейф systemd-юнитов и Orca-автоматизаций от текущих specs**:
  ```
  secretary upgrade --instance "${SECRETARY_INSTANCE:-$HOME/secretary-instance}" --dry-run --no-pull
  ```
