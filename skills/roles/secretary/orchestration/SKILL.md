...
## Headless Serve And Remote Client Visibility

On a headless `orca serve` runtime (no renderer window; the user watches through a desktop or web Orca client connected to it as a remote runtime), terminal visibility differs from a local app. The connected client is the renderer, and the server cannot push tabs into it directly.

- Server-side `orca terminal create` returns `surface: background` with a "could not make it discoverable" warning. Without a main window there is no `revealTerminalSession` notifier, so no renderer adopts the tab. `--focus` fails with "No renderer window available", and `orca terminal focus` does not surface it to the remote client.
- Worktree-level state syncs to the remote client automatically. The client materializes a worktree's terminals (via `session.tabs.list` / `session.tabs.activate`) only when it activates/opens that worktree. After it opens, input and output round-trip normally.
- A terminal created in a worktree the user already has open does not push to the client; it only fetches on activation.

To spawn a worker the remote user can see and drive, create it as a NEW worktree, then have the user open it:

```bash
orca worktree create --repo id:<repoId> --name <task> --no-parent --agent <agent> --prompt "<brief>" --json
# then ask the user to open the new worktree in their client
```

For CLI-only workers you drive yourself (the user need not see them), background `orca terminal create` handles are fine.

## Reading Worker Activity (Read-Only)
...
```bash
python3 - "$WORKER_CWD" <<'PY'
import json, sys, glob, os
proj = os.path.expanduser("~/.claude/projects/-" + sys.argv[1].strip("/").replace("/", "-"))
f = max(glob.glob(proj + "/*.jsonl"), key=os.path.getmtime)
for line in open(f, errors="replace").read().splitlines()[-40:]:
    try: d = json.loads(line)
    except ValueError: continue
    c = (d.get("message") or {}).get("content")
    c = [{"type": "text", "text": c}] if isinstance(c, str) else (c or [])
    for b in c:
        if not isinstance(b, dict): continue
        if b.get("type") == "text" and b.get("text", "").strip():
            print(("USER" if d.get("type") == "user" else "ASST") + ": " + " ".join(b["text"].split())[:300])
        elif b.get("type") == "tool_use":
            print("  -> " + b.get("name", "") + " " + json.dumps(b.get("input", {}), ensure_ascii=False)[:120])
PY
```
