...
def _review_baseline(value: Any) -> int:
    ...
    if value < 0 or value > MAX_REVIEW_BASELINE:
        raise _ContextDamage(f"review context review_baseline must be between 0 and {MAX_REVIEW_BASELINE}")
    return value
...
class ValidatedReviewIdentity:
    ...
    def __post_init__(self) -> None:
        if self.seal is not _POST_VERDICT_AUTHORITY:
            raise ValueError("a validated review identity is issued only by validate_post_verdict_identity")
