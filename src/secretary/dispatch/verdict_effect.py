...
3. resolve the current pair — the checkout as it is now and the base branch as it is now —
   through ``current_pair``, which answers with an exact pair or with a typed failure;
...
Step 3 is typed for a reason this executor got wrong once. It used to read the two revisions with
host calls that answer ``""`` whenever Git cannot be run, and step 4 compared those strings: an
empty side matched everything, so an unreadable workspace and an intact one produced the same
verdict about drift, and a card could be parked or merged over a pair nobody had read — on the
...
class EffectPreconditions:
    ...
    def __post_init__(self) -> None:
        ...
        # The second half of the same enforcement. The seal says this chain ran; this says the chain
        # resolved an exact pair rather than carrying a read it could not make. Only a success has
        # the type, so an unresolved pair cannot be smuggled in as a precondition. Refused the same
        # way a broken seal is, because to a caller they are one condition: these preconditions were
        # not established.
        if type(self.current) is not ResolvedCurrentPair:
            raise ValueError("effect preconditions require a successfully resolved current pair")

    @property
    def checkout_sha(self) -> str:
        """The candidate the effect is about to act on, as this invocation resolved it."""
        return self.current.candidate_sha

    @property
    def base_sha(self) -> str:
        """The base branch the effect is about to land on, as this invocation resolved it."""
        return self.current.base_sha
    ...
    def of(cls, task: dict[str, Any]) -> _MechanicalOutcomes:
        if str(task.get("state") or "") == "assessment":
            return cls(
                "assessment",
                True,
                "release-gate-transport-blocked",
                "release-current-pair-blocked",
                "Observer decision: release. ",
            )
        return cls("review", False, "merge-gate-transport-blocked", "review-current-pair-blocked", "")


def standing_verdict(
    runtime: Any, task: dict[str, Any], record: DispatcherRecord
) -> VerdictProjection | None:
    ...
    for raw in reversed(runtime.audit.events(task["ref"])):
        ...
        if any(comment.get("marker") == marker and comment.get("body") == rendered for comment in comments):
            return projection
    return None
...
def _establish_preconditions(
    ...
    if not isinstance(identity, ValidatedReviewIdentity):
        return identity
    current = resolve_current_pair(runtime.host, task, record, identity)
    if isinstance(current, UnresolvedCurrentPair):
        # No pair was read, so there is nothing to compare and nothing an effect may stand on. This
        # is not drift and is never answered as one: a card is not reworked because a host could
        # not run Git. It blocks, naming the typed outcome, so an operator repairs the read.
        return _block_unresolved(runtime, task, record, records, payload, attempt_id, outcomes, current)
    drift = _drift(identity, current)
    if drift:
    ...
    blocked = runtime._accept_green_gate(task, record, records, payload, attempt_id, result, stage=stage)
...
def _drift(identity: ValidatedReviewIdentity, current: ResolvedCurrentPair) -> str:
    """Has the code this verdict describes moved? The operator message, or "" when it has not.

    The whole resolved pair against the whole sealed pair, and both halves are the same question.
    The checkout must still hold the candidate the reviewer was pointed at, or the effect would
    land work nobody read. And the base branch must still contain the base the round was judged
    over: a base that advanced is the ordinary case and keeps it as an ancestor, while a base that
    was rewritten leaves the reviewed delta describing a history that no longer exists.

    A pure comparison, and that is the repair. The previous version read the pair itself, from a
    pair of host calls that answered "" when they failed, and an empty side of a comparison
    silently matched everything. Here every input has already been resolved exactly or the caller
    never reached this line, so there is no value this function can be handed that means "unknown".
    """
    if not current.candidate_intact:
        return (
            f"The review was given for commit `{identity.candidate_sha[:12]}` while the working copy "
            f"is now on `{current.candidate_sha[:12]}`: the verdict describes a different state of "
            f"the code. The card is back in In progress; rework it and report again."
        )
    if not current.base_intact:
        return (
            f"The review was given over base `{identity.base_sha[:12]}`, which the base branch at "
            f"`{current.base_sha[:12]}` no longer descends from: the history the verdict read has "
            f"been replaced, so its delta no longer exists. The card is back in In progress; rework "
            f"it and report again."
        )
    return ""
...
def _block_unresolved(
    ...
    attempt_id: str,
    outcomes: _MechanicalOutcomes,
    unresolved: UnresolvedCurrentPair,
) -> dict[str, Any]:
    """The one answer to a current pair that could not be resolved, on either side of the seam.

    Blocked, always, and with the typed outcome written into the card. Not a bounce to the worker:
    a candidate nobody could read is not evidence that a worker did anything wrong, and handing the
    round back would spend a rework on a host failure. Not a retry either: the resolver's failures
    are conditions somebody has to change — a workspace that is gone, a Git that will not run, a
    ref that names two revisions — and a tick that quietly repeated them would burn the card's
    budget without ever saying what is wrong.

    No gate has run at this point and no receipt is written, which is the property the criterion is
    about: a stage gate executed here would attest a checkout nobody could name.
    """
    return runtime._block_merge_path(
        task,
        record,
        ...
        payload,
        attempt_id,
        action=outcomes.pair_action,
        reason=f"{outcomes.reason_prefix}{unresolved.evidence}",
        step=outcomes.step,
        outcome=f"current pair {unresolved.outcome}",
    )
...
def _receipt_mismatch(
    ...
    if receipt is None:
        return ""
    if receipt.validated_sha != identity.candidate_sha or receipt.validated_sha != current.candidate_sha:
        return (
