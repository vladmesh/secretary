...
reaches it from outside this host arrives through Caddy, which terminates TLS and authenticates the
owner before proxying to `127.0.0.1`. There is no second way in, because the application still
cannot be bound anywhere a second way could reach.
...
* :mod:`secretary.webfront.guard` reads a rendered configuration back and answers which of the
  transport's published routes it would serve without authenticating the owner. That is a parser
