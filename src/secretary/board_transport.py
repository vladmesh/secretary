...
def findings(instance_dir: Path | str) -> list[str]:
    ...
    The board is the PostgreSQL store, reached through `board-store.env` with its own evidence in
    `board.store.findings`, so the JSON-RPC tuple is no transport of it: an absent or unusable one
    is not a defect.  What stays a finding is the file being *tracked* in the instance repository,
    which is a statement about the repository rather than about reachability.
    """
    path = transport_path(instance_dir)
