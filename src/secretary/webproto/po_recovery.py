...
def recover_po_turns(instance: str | Path, data_dir: str | Path | None) -> list[str]:
    """Settle the PO turns a previous run of this service left `running`; lines for the journal.

    The dashboard does not depend on the PO store, so a store that cannot be reached becomes a line
    ...
    try:
        ...
        recovered = PoRunner.for_instance(instance, data_dir).recover()
