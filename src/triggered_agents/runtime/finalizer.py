...
def spawn_finalizer(agent: str, generation: int | None = None) -> int:
    ...
    try:
        ...
        state.log_run("finalize", action="self-teardown-helper-failed", generation=generation, error=str(exc))
...
def _finalize_locked(
    agent: str, ws: str, state: AgentState, generation: int | None, *, host: SessionHost
) -> int:
    ...
    if generation is not None and live_generation is not None and live_generation != generation:
        ...
        print(
            f"dispatch[{agent}]: finalize — terminal already superseded by a newer run"
    ...
    if dispatch._stop_and_confirm_workspace_empty(ws, host=host):
        ...
        if ok:
            ...
            print(
                f"dispatch[{agent}]: finalize — stopped the pty but a ghost tab would not close; "
                "next tick re-reaps"
            )
    else:
    ...
    for attempt in range(dispatch.FINALIZE_LOCK_ATTEMPTS):
        try:
            ...
            if attempt + 1 >= dispatch.FINALIZE_LOCK_ATTEMPTS:
                ...
                print(
                    f"dispatch[{agent}]: finalize deferred — a live tick holds the lock; it or "
