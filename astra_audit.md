...
- 🟡 **A14/A15 advanced across PR #463–#473:** #463 established the narrow `secretary.dispatch` construction/command boundary and import ratchet; #464–#470 moved all dispatcher satellites into the feature package without flat forwarding modules. #471 extracted Ready-card claim/admission/preflight into `dispatch.claim`; #472 extracted post-claim bring-up and headless recovery into `dispatch.worker_launch`. #473 now owns report marker selection, done/blocked acceptance, stale/infra report handling, and the one-shot report nudge in `dispatch.worker_report`. The runtime orchestrates the existing launch, retained-continuation, report and generic-wait boundaries; durable JSON, request identities and save/effect ordering are unchanged. Architecture, collaborator-surface and incremental typecheck gates cover the new module. A14/A15 remain open for retained/red continuation, shared wait/vitality and later gate/review/merge extractions, one coherent state machine at a time.
...
At audit time, `src/secretary/dispatcher.py` was about 324 KB and mixed composition, compatibility re-exports and lifecycle implementations. After PR #473 it is about 242 KiB / 5,752 lines; construction is package-owned, all flat dispatcher satellites are gone, and claim, worker launch/recovery and worker report handling have explicit package owners. The remaining compatibility/state-machine facade is still large and needs bounded internal extractions rather than another satellite-file move.
...
- `dispatcher.py` ~242 KiB / 5,752 lines after #473;
- `tasks.py` ~221 KB;
...
PR #458 completed lifecycle `worker_head_run` / `review_head_run`, #460 completed routing snapshots, #461 completed gate identity and delivery evidence, and #462 completed `launch_intent` plus `worker_headless`, preserving durable JSON. #463 established the package construction boundary; #464–#470 completed the move-only satellite migration. #471 extracted claim/admission/preflight; #472 extracted worker bring-up and headless recovery; #473 extracted worker report acceptance, stale/infra cases and the one-shot report nudge. The root dispatcher is now about 242 KiB / 5,752 lines. The next coherent candidate is **retained/red worker continuation**: transition intent, board rework move, delivery/recovery and retained-resume or replacement. Do not fold the generic wait/vitality policy or the larger gate/review/merge lifecycle into that same PR. Internal splits of already-package-owned `dispatch.production` and `dispatch.observer` remain separate concerns.
...
With A01–A13 and A16 complete and A14/A15 advanced through PR #473, the move-only dispatcher satellite migration is complete and three lifecycle responsibilities have package owners: `dispatch.claim`, `dispatch.worker_launch` and `dispatch.worker_report`. Continue extracting one bounded responsibility from `DispatcherRuntime` at a time; retained/red continuation is the next coherent candidate, separate from shared wait/vitality and gate/review/merge. A17/A19/A20 remain explicit deprecation projects with migration/rollback criteria, not additions to an ordinary behavior-preserving refactor.
