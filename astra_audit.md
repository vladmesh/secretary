...
- 🟡 **A14/A15 advanced in PR #463:** extracted runtime construction/data-dir resolution to `secretary.dispatch.bootstrap`, moved the flat `dispatcher_commands.py` module to `secretary.dispatch.commands`, rewired the production callers, and added an architecture ratchet that allows production imports of the legacy `secretary.dispatcher` monolith only from the narrow bootstrap boundary. `DispatcherRuntime` itself intentionally remains in the monolith, so both findings remain open for further bounded extraction.
...
| A14 | 🟡 Remove the giant `dispatcher.py` compatibility façade — **first bootstrap/consumer slice landed in #463** | **4** |
| A15 | 🟡 Continue the flat-root-to-feature-package migration and split god modules — **`dispatcher_commands.py` moved in #463** | **4** |
