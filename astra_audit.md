...
- ◐ **A12 partially completed in PR #450:** introduced immutable typed payload values for the three control-plane Card marker events (`card.reported`, `card.verdict`, `card.decided`) and normalized their released dict payloads once at the `MarkerComment` host boundary. The exact event-data dictionary projection, marker grammar, request/event identity, Kanboard replay/recovery behavior, and historical decision evidence stay unchanged. `attempt.usage`, `attempt.outcome`, outcome-round context, and other closed event payloads remain for later bounded A12 slices.
...
| A12 | ◐ Replace closed event payload dictionaries with typed payload objects — **marker payload slice completed in #450; usage/outcome/context remain** | **4** |
...
**Remaining A12:** type the next closed event payloads in similarly bounded slices. The highest-value candidates are `AttemptUsagePayload`, `AttemptOutcomePayload`, and `OutcomeRoundContext`; keep an escape hatch for opaque/legacy event data and do not attempt to model every historical payload in one migration.
...
With A01–A11 and A16 complete and the first A12 marker-payload slice merged, the highest-value near-term sequence is: **finish the remaining bounded A12 payloads → A13**, keeping each migration to one closed domain shape at a time. A17/A19/A20 should be planned as explicit deprecation projects rather than mixed into ordinary refactors.
