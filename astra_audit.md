...
| A14 | 🟡 Remove the giant `dispatcher.py` compatibility façade — **bootstrap boundary landed in #463; pause domain moved in #464; mechanical gate boundary moved in #465** | **4** |
| A15 | 🟡 Continue the flat-root-to-feature-package migration and split god modules — **`dispatcher_commands.py` moved in #463; pause modules moved in #464; gate modules moved in #465** | **4** |
...
With A01–A13 and A16 complete and A14/A15 underway, the highest-value near-term work is to **continue A14/A15 in bounded package-boundary slices**, using the new `secretary.dispatcher` import ratchet and removing flat-root entries as their implementations move. A17/A19/A20 should still be planned as explicit deprecation projects rather than mixed into ordinary refactors.