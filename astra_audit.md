...
- 🟡 **A13 advanced across PR #458, PR #460, and PR #461:** #458 typed durable lifecycle `worker_head_run` / `review_head_run`; #460 moved durable routing `worker_run` / `review_run` behind `PersistedRoutingHeadSnapshot` with canonical `RoutingHeadSnapshot` views; #461 moved gate receipt/PR-authorship/publication identity and worker/reviewer delivery evidence behind typed persisted boundaries while preserving exact historical JSON projections. The remaining A13 families are `launch_intent` and headless recovery state.
...
| A13 | 🟡 Break up `DispatcherRecord`'s nested `dict[str, Any]` state — **lifecycle + routing + gate identity/delivery slices completed in #458/#460/#461; launch intent + headless remain** | **4** |
...
`DispatcherRecord` itself is a dataclass. After the lifecycle, routing, gate-identity, and delivery-evidence slices, the remaining important durable state families that are still untyped bags are:
...
**Next bounded slice:** type `launch_intent` as its own A13 cluster because it spans pre-launch persistence, worker/reviewer recovery, and provider-bound identity. After that, type `worker_headless`; those two slices complete the currently inventoried A13 durable bags. The previously audited `gate_workflow_dispatch` bag is already gone from current `main` and remains outside the inventory.
...
PR #458 completed lifecycle `worker_head_run` / `review_head_run`, PR #460 completed routing `worker_run` / `review_run`, and PR #461 completed gate receipt/PR-authorship/publication identity plus worker/reviewer delivery evidence, all without changing durable JSON. Next take `launch_intent`, then `worker_headless`, as separate bounded PRs. Keep A14/A15 behind those state boundaries so package extraction does not simply move raw dictionaries into new modules.
...
With A01–A12 and A16 complete and A13 underway, the highest-value near-term sequence is: **A13 launch intent → headless recovery → A14/A15**, keeping each migration to one durable domain shape at a time. A17/A19/A20 should still be planned as explicit deprecation projects rather than mixed into ordinary refactors.