...
- ◐ **A12 partially completed across PR #450 and PR #451:** #450 introduced immutable typed payload values for the three control-plane Card marker events (`card.reported`, `card.verdict`, `card.decided`). #451 added immutable `AttemptUsagePayload`, `AttemptUsagePhase`, and `TokenAccount` values, made canonical attempt-usage occurrences carry the typed payload, and moved causal predecessor/boundary reads off raw event dictionaries. Released event-data dictionaries, request/event identity, committed+pending projection semantics, token attribution arithmetic, provider-journal behavior, marker grammar, and recovery compatibility remain unchanged. `attempt.outcome`, outcome-round context, and other closed event payloads remain for later bounded A12 slices.
...
| A12 | ◐ Replace closed event payload dictionaries with typed payload objects — **marker payloads completed in #450; attempt usage completed in #451; outcome/context remain** | **4** |
...
**Remaining A12:** continue with the other high-value closed schemas, especially `AttemptOutcomePayload` and `OutcomeRoundContext`. Keep the generic `Event.data` escape hatch for genuinely opaque/legacy data rather than trying to model every historical payload in one migration.
...
With A01–A11 and A16 complete and the marker plus `attempt.usage` A12 slices merged, the highest-value near-term sequence is: **finish `AttemptOutcomePayload` / `OutcomeRoundContext` → A13**, keeping each migration to one closed domain shape at a time. A17/A19/A20 should be planned as explicit deprecation projects rather than mixed into ordinary refactors.
