...
- 🟡 **A14/A15 advanced across PR #463, PR #464, PR #465, PR #466, PR #467, PR #468, PR #469, PR #470, PR #471, and PR #472:** #463 established the narrow `secretary.dispatch` construction/command boundary and import ratchet; #464–#470 moved pause, gate, observer, production, shared state/types, head observation, and launch/review satellite ownership into the feature package with the flat dispatcher satellites removed. #471 made the first internal extraction from `DispatcherRuntime`: Ready-card head selection, sprint admission, broad-check/Git-access preflight, and durable claim orchestration now live in `secretary.dispatch.claim` behind typed `ClaimHandoff`. #472 continued directly across that seam: durable claim → worker bring-up, shared worker relaunch/failure handling, and headless-worker recovery/refusal/replacement now live in `secretary.dispatch.worker_launch`; `dispatch.claim` calls that package boundary directly, later report/red-continuation/gate/review/merge state machines remain in `DispatcherRuntime`, and architecture/typecheck/collaborator-surface ratchets cover the new module. Both findings remain open for later bounded internal extractions.
...
| A14 | 🟡 Remove the giant `dispatcher.py` compatibility façade — **bootstrap #463; pause #464; gate #465; observer #466; production #467; shared state/model layer #468; head observation #469; launch/review lifecycle #470; claim/admission/preflight #471; worker bring-up/headless recovery #472** | **4** |
| A15 | 🟡 Continue the flat-root-to-feature-package migration and split god modules — **commands #463; pause #464; gate #465; observer #466; production #467; state/types/worker-lifecycle #468; heartbeat/watchdog/TUI #469; helpers/launch/launcher/review #470; internal claim extraction #471; internal worker-launch/headless extraction #472** | **4** |
...
- `dispatcher.py` ~268 KiB after #472;
- `tasks.py` ~221 KB;
...
PR #458 completed lifecycle `worker_head_run` / `review_head_run`, PR #460 completed routing `worker_run` / `review_run`, PR #461 completed gate receipt/PR-authorship/publication identity plus worker/reviewer delivery evidence, and PR #462 completed `launch_intent` plus `worker_headless`, all without changing durable JSON. PR #463 then started A14/A15 package-boundary work; #464–#470 completed the move-only dispatcher satellite migration. PR #471 made the first internal extraction from `DispatcherRuntime`: Ready-card claim/admission/preflight now lives in `secretary.dispatch.claim`. PR #472 continued across the typed `ClaimHandoff`: claim-to-running-worker bring-up, common worker relaunch/failure handling, and headless-worker recovery now live in `secretary.dispatch.worker_launch`. `src/secretary/dispatcher.py` remains the only flat-root `dispatcher*.py` module, now about 268 KiB / 6.3k lines. The next A14/A15 work should still take one coherent state-machine responsibility at a time; later worker report/wait/retained-continuation logic and the larger gate/review/merge lifecycle should not be collapsed into one PR. Internal splits of already-package-owned large modules such as `secretary.dispatch.production` or `secretary.dispatch.observer` remain separate follow-up concerns.
...
With A01–A13 and A16 complete and A14/A15 advanced through PR #472, the move-only dispatcher satellite migration is complete and the internal split has two package-owned state-machine boundaries: `secretary.dispatch.claim` owns Ready-to-durable-claim admission/preflight, and `secretary.dispatch.worker_launch` owns durable-claim-to-running-worker bring-up plus headless-worker recovery. The highest-value near-term A14/A15 work remains to **extract bounded later lifecycle responsibilities from `DispatcherRuntime` into `secretary.dispatch`**, one coherent boundary at a time, without folding the report/red-continuation/gate/review/merge lifecycle into one radical PR. A17/A19/A20 should still be planned as explicit deprecation projects rather than mixed into ordinary refactors.