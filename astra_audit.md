...
| A14 | 🟡 Remove the giant `dispatcher.py` compatibility façade — **bootstrap #463; pause #464; gate #465; observer #466; production #467; shared state/model layer #468; head observation #469; launch/review lifecycle #470** | **4** |
| A15 | 🟡 Continue the flat-root-to-feature-package migration and split god modules — **commands #463; pause #464; gate #465; observer #466; production #467; state/types/worker-lifecycle #468; heartbeat/watchdog/TUI #469; helpers/launch/launcher/review #470** | **4** |
...
With A01–A13 and A16 complete and A14/A15 advanced through PR #470, the move-only dispatcher satellite migration is effectively complete: `src/secretary/dispatcher.py` is now the only remaining flat-root `dispatcher*.py` module. The highest-value near-term A14/A15 work is to **extract bounded state-machine responsibilities from `DispatcherRuntime` into `secretary.dispatch`**, one coherent dependency boundary at a time, rather than moving the monolith wholesale. A17/A19/A20 should still be planned as explicit deprecation projects rather than mixed into ordinary refactors.