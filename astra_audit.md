...
- 🟡 **A13 advanced across PR #458 and PR #460:** #458 typed durable lifecycle `worker_head_run` / `review_head_run`; #460 moved durable routing `worker_run` / `review_run` behind `PersistedRoutingHeadSnapshot` with canonical `RoutingHeadSnapshot` views, removed the old dict-returning launch enrichment wrapper, and added both routing/state modules to the incremental mypy gate while preserving exact historical JSON projections. The remaining A13 families are gate receipts/publication identity, delivery evidence, launch intent, and headless recovery state.
...
| A13 | 🟡 Break up `DispatcherRecord`'s nested `dict[str, Any]` state — **lifecycle + routing slices completed in #458/#460** | **4** |
...
### A13. Break up `DispatcherRecord`'s nested dictionaries — complexity 4 — in progress

`DispatcherRecord` itself is a dataclass. After the lifecycle and routing slices, the remaining important durable state families that are still untyped bags are:

- `launch_intent`;
- gate receipts / publication identity (`gate_attestation`, `gate_pr_authorship`, `gate_published_ref`);
- delivery evidence (`worker_delivery_evidence` / `review_delivery_evidence`);
- headless recovery state (`worker_headless`).

The runtime already has typed `HeadRun`, `StopInitiator`, continuation values, vitality episode values, and A10's typed `RoutingHeadSnapshot`, so keeping the remaining persisted forms as dicts inside the in-memory domain record still throws away type safety exactly where the dispatcher is most stateful.
...
**Next bounded slice:** type the gate receipt/publication identity family (`gate_attestation`, `gate_pr_authorship`, `gate_published_ref`) as one coherent A13 cluster, preserving the released JSON and recovery semantics. After that, take delivery evidence, launch intent, and headless recovery state as separate bounded slices rather than one large rewrite. The previously audited `gate_workflow_dispatch` bag is already gone from current `main` and remains outside the inventory.

Backward-compatibility/restart coverage remains mandatory for every A13 slice because these fields survive dispatcher restarts.
...
With A01–A12 and A16 complete and A13 underway, the highest-value near-term sequence is: **A13 gate receipt/publication identity → delivery/launch/headless state → A14/A15**, keeping each migration to one durable domain shape at a time. A17/A19/A20 should still be planned as explicit deprecation projects rather than mixed into ordinary refactors.