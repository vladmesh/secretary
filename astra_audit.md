...
| A14 | 🟡 Remove the giant `dispatcher.py` compatibility façade — **bootstrap #463; pause #464; gate #465; observer #466; production #467; shared state/model layer #468; head observation #469** | **4** |
| A15 | 🟡 Continue the flat-root-to-feature-package migration and split god modules — **commands #463; pause #464; gate #465; observer #466; production #467; state/types/worker-lifecycle #468; heartbeat/watchdog/TUI #469** | **4** |
...
PR #458 completed lifecycle `worker_head_run` / `review_head_run`, PR #460 completed routing `worker_run` / `review_run`, PR #461 completed gate receipt/PR-authorship/publication identity plus worker/reviewer delivery evidence, and PR #462 completed `launch_intent` plus `worker_headless`, all without changing durable JSON. PR #463 then started A14/A15: runtime construction now lives in `secretary.dispatch.bootstrap`, dispatcher CLI handlers live in `secretary.dispatch.commands`, the old flat command module is gone, and an architecture ratchet prevents new production consumers of the monolith. PR #464 moved the pause domain into `secretary.dispatch`; PR #465 moved the mechanical gate and gate-receipt boundary there; PR #466 moved the observer lifecycle and observer fence there; PR #467 moved the production loop there as a move-only package-ownership change; PR #468 moved the shared dispatcher state/types/worker-lifecycle layer there and removed the three corresponding flat-root modules. Next continue with another coherent dispatcher boundary rather than moving `DispatcherRuntime` wholesale. The most natural next bounded candidate is the heartbeat/watchdog identity seam (`dispatcher_heartbeat.py` + `dispatcher_watchdog.py`), now that its state dependency is package-local; the behavior-heavy launch/review modules should remain later slices. `secretary.dispatch.production` remains large enough to split internally later, but that should be a separate concern from continuing the flat-root migration.
...
With A01–A13 and A16 complete and A14/A15 underway through PR #468, the highest-value near-term work is to **continue A14/A15 in bounded package-boundary slices**, using the `secretary.dispatcher` import ratchet and removing flat-root entries as their implementations move. The next candidate is the heartbeat/watchdog identity seam; `dispatcher_launch.py` / `dispatcher_review.py` remain intentionally later because they carry substantially more behavior. A17/A19/A20 should still be planned as explicit deprecation projects rather than mixed into ordinary refactors.