...
**Fix:** deserialize JSON into typed sub-values in `DispatcherRecord.from_json()` and serialize only in `to_json()`. Start with `worker_head_run` / `review_head_run`, because canonical runtime `HeadRun` parsing already exists and this removes a high-value dict seam without changing the durable document. Then take `worker_run` / `review_run`, gate receipt/publication identity, and delivery/launch/headless documents as separate bounded A13 slices rather than one large rewrite. The previously audited `gate_workflow_dispatch` bag is already gone from current `main` and is therefore removed from the remaining A13 inventory.
...
A12 is complete; continue with A13, then A14 and A15.

A12 is complete after PR #457. Begin A13 with the existing runtime `HeadRun` boundary for `worker_head_run` / `review_head_run`, then migrate the remaining durable state families in separate PRs. Keep A14/A15 behind those state boundaries so package extraction does not simply move raw dictionaries into new modules.
...
With A01–A12 and A16 complete, the highest-value near-term sequence is: **A13 `worker_head_run` / `review_head_run` → remaining A13 state families → A14/A15**, keeping each migration to one durable domain shape at a time. A17/A19/A20 should still be planned as explicit deprecation projects rather than mixed into ordinary refactors.
