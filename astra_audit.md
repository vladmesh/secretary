...
- 🟡 **A13 advanced in PR #458:** `DispatcherRecord.worker_head_run` / `review_head_run` now cross a dedicated `PersistedHeadRun` boundary that parses complete modern records once into canonical lifecycle `HeadRun` values while preserving the exact released JSON mapping and historical/minimal records. Existing dict-oriented callers remain compatible during the staged migration. The remaining A13 families are routing snapshots (`worker_run` / `review_run`), gate receipts/publication identity, delivery evidence, launch intent, and headless recovery state.
...
| A13 | 🟡 Break up `DispatcherRecord`'s nested `dict[str, Any]` state — **lifecycle head-run slice completed in #458** | **4** |
...
`DispatcherRecord` itself is a dataclass, but several important durable state families are still untyped bags:
...
**Next bounded slice:** migrate `worker_run` / `review_run` onto A10's existing `RoutingHeadSnapshot` model, again preserving the durable JSON projection. That is now the cleanest adjacent cluster because the lifecycle and routing concepts are already separated and named canonically. After that, take gate receipt/publication identity and delivery/launch/headless documents as separate A13 slices rather than one large rewrite. The previously audited `gate_workflow_dispatch` bag is already gone from current `main` and remains outside the inventory.
...
PR #458 completed the lifecycle `worker_head_run` / `review_head_run` slice without changing durable JSON. Next migrate routing telemetry `worker_run` / `review_run` onto `RoutingHeadSnapshot`; then take gate receipt/publication identity and delivery/launch/headless state as separate bounded PRs. Keep A14/A15 behind those state boundaries so package extraction does not simply move raw dictionaries into new modules.
...
With A01–A12 and A16 complete and A13 underway, the highest-value near-term sequence is: **A13 `worker_run` / `review_run` → remaining A13 state families → A14/A15**, keeping each migration to one durable domain shape at a time. A17/A19/A20 should still be planned as explicit deprecation projects rather than mixed into ordinary refactors.