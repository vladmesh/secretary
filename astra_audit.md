...
- ◐ **A12 partially completed across PR #450, PR #451, and PR #452:** #450 introduced immutable typed payload values for the three control-plane Card marker events (`card.reported`, `card.verdict`, `card.decided`). #451 added immutable `AttemptUsagePayload`, `AttemptUsagePhase`, and `TokenAccount` values, made canonical attempt-usage occurrences carry the typed payload, and moved causal predecessor/boundary reads off raw event dictionaries. #452 added immutable `AttemptOutcomePayload` plus typed source-event ids, usage-completeness, lineage-requiredness, verdict, and terminal-state values; canonical attempt-outcome occurrences now carry the typed payload and offline analytics/natural-key reads consume it instead of indexing raw outcome dictionaries. Released event-data dictionaries, request/event identity, checkpoint projection shape, lifecycle semantics, marker grammar, and recovery compatibility remain unchanged. `OutcomeRoundContext` remains the clearest next bounded A12 slice.
...
| A12 | ◐ Replace closed event payload dictionaries with typed payload objects — **marker payloads completed in #450; attempt usage in #451; attempt outcome in #452; outcome-round context remains** | **4** |
...
### A12. Replace closed event payload dicts with typed payload objects — complexity 4 — in progress
...
**Remaining A12:** `OutcomeRoundContext` is the clearest next closed schema. Keep the generic `Event.data` escape hatch for genuinely opaque/legacy data rather than trying to model every historical payload in one migration.
...
With A01–A11 and A16 complete and the marker, `attempt.usage`, and `attempt.outcome` A12 slices merged, the highest-value near-term sequence is: **`OutcomeRoundContext` → A13**, keeping each migration to one closed domain shape at a time. A17/A19/A20 should be planned as explicit deprecation projects rather than mixed into ordinary refactors.
