...
- 🟡 **A14/A15 advanced across PR #463, PR #464, PR #465, PR #466, PR #467, PR #468, PR #469, PR #470, and PR #471:** #463 extracted runtime construction/data-dir resolution to `secretary.dispatch.bootstrap`, moved the flat `dispatcher_commands.py` module to `secretary.dispatch.commands`, rewired production callers, and added the narrow `secretary.dispatcher` import ratchet. #464 then moved the complete pause state/operation domain from `dispatcher_pause.py` / `dispatcher_pause_ops.py` to `secretary.dispatch.pause` / `secretary.dispatch.pause_ops`, rewired production/web/test callers, and removed both flat-root modules without forwarding shims. #465 moved the mechanical gate and exact-SHA gate receipt from `dispatcher_gate.py` / `dispatcher_gate_receipt.py` to `secretary.dispatch.gate` / `secretary.dispatch.gate_receipt`, rewired all production/test consumers, and removed both flat modules without forwarding shims. #466 moved the observer lifecycle and pre-advance observer fence from `dispatcher_observer.py` / `dispatcher_observer_fence.py` to `secretary.dispatch.observer` / `secretary.dispatch.observer_fence`, rewired production/protocol/test consumers, and removed both flat modules without forwarding shims. #467 moved the production loop from `dispatcher_production.py` to `secretary.dispatch.production`, rewired runtime/web/telemetry/test consumers, and removed the flat module without a forwarding shim. #468 moved the shared dispatcher state/model layer from `dispatcher_state.py`, `dispatcher_types.py`, and `dispatcher_worker_lifecycle.py` to `secretary.dispatch.state`, `secretary.dispatch.types`, and `secretary.dispatch.worker_lifecycle`, rewired production/tests/mocks/typecheck, and removed all three flat modules without forwarding shims while preserving durable JSON and lifecycle semantics. #469 moved `dispatcher_heartbeat.py`, `dispatcher_watchdog.py`, and `dispatcher_tui.py` to `secretary.dispatch.heartbeat`, `secretary.dispatch.watchdog`, and `secretary.dispatch.tui`. #470 moved the remaining launch/review satellite modules to `secretary.dispatch`. #471 then started the internal state-machine split: Ready-card head selection, sprint admission, broad-check/Git-access preflight, and durable claim orchestration now live in `secretary.dispatch.claim`, with typed `ClaimHandoff` separating the durable claim from worker bring-up. Production calls that package boundary directly; `dispatcher.py` retains only narrow compatibility forwarding for `_claim`, `resolve_head`, and the historical admission constants. `DispatcherRuntime` still owns worker bring-up/recovery and the later lifecycle state machines, so both findings remain open for further bounded extraction.
...
| A14 | 🟡 Remove the giant `dispatcher.py` compatibility façade — **bootstrap #463; pause #464; gate #465; observer #466; production #467; shared state/model layer #468; head observation #469; launch/review lifecycle #470; claim/admission/preflight #471** | **4** |
| A15 | 🟡 Continue the flat-root-to-feature-package migration and split god modules — **commands #463; pause #464; gate #465; observer #466; production #467; state/types/worker-lifecycle #468; heartbeat/watchdog/TUI #469; helpers/launch/launcher/review #470; first internal claim extraction #471** | **4** |
...
- `dispatcher.py` ~301 KB after #471;
- `tasks.py` ~221 KB;
- `dispatch/host.py` ~221 KB;
- `sprints.py` ~159 KB;
- `dispatch/observer.py` ~152 KB (moved from the flat root in #466; still a later split candidate);
...
A13 is complete. A14/A15 are now active and should continue as bounded package-boundary extractions rather than a wholesale move of `DispatcherRuntime`.

PR #458 completed lifecycle `worker_head_run` / `review_head_run`, PR #460 completed routing `worker_run` / `review_run`, PR #461 completed gate receipt/PR-authorship/publication identity plus worker/reviewer delivery evidence, and PR #462 completed `launch_intent` plus `worker_headless`, all without changing durable JSON. PR #463 then started A14/A15: runtime construction now lives in `secretary.dispatch.bootstrap`, dispatcher CLI handlers live in `secretary.dispatch.commands`, the old flat command module is gone, and an architecture ratchet prevents new production consumers of the monolith. PR #464 moved the pause domain into `secretary.dispatch`; #465 moved the mechanical gate and gate-receipt boundary there; #466 moved the observer lifecycle and observer fence there; #467 moved the production loop; #468 moved shared state/types/worker-lifecycle; #469 moved heartbeat/watchdog/TUI; and #470 moved helpers/launch/launcher/review. PR #471 then made the first internal extraction from `DispatcherRuntime`: the complete Ready-card claim/admission/preflight responsibility now lives in `secretary.dispatch.claim`, ending at a typed `ClaimHandoff` before worker bring-up. `src/secretary/dispatcher.py` remains the only `dispatcher*.py` module at the flat root, but is now about 30 KB smaller and no longer owns claim decision logic. The next A14/A15 work should continue one coherent state-machine boundary at a time; the natural adjacent seam is worker bring-up/recovery around `_launch_worker_after_claim`, while the much larger gate/red-continuation/merge path should stay a separate later slice. Internal splits of already-package-owned large modules such as `secretary.dispatch.production` or `secretary.dispatch.observer` remain separate follow-up concerns.
...
With A01–A13 and A16 complete and A14/A15 advanced through PR #471, the move-only dispatcher satellite migration is complete and the internal split has started: `src/secretary/dispatcher.py` is the only remaining flat-root `dispatcher*.py` module, while claim/admission/preflight already belongs to `secretary.dispatch.claim`. The highest-value near-term A14/A15 work remains to **extract bounded state-machine responsibilities from `DispatcherRuntime` into `secretary.dispatch`**, one coherent dependency boundary at a time. The adjacent worker bring-up/recovery seam is now a better-sized next candidate than attempting the entire gate/red-continuation/merge lifecycle in one PR. A17/A19/A20 should still be planned as explicit deprecation projects rather than mixed into ordinary refactors.