...
class WorkerContinuationBoundaryTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.record = DispatcherRecord(
            worker="worker-1", workspace="/unused", handle="worker-1", head="codex",
            review_head="claude", attempt_id="held-attempt", comment_baseline=0,
            review_baseline=0, state="validate", claimed_at=1.0, report_generation=3, attempt_round=1,
        )
        self.record.worker_head_run = {
            "run_id": "retained-run", "workspace": "/unused",
            "task_ref": {"kind": "card", "ref": "sample-1"},
            "role": "worker", "spec": {"profile_id": "codex", "adapter": "codex"},
        }
    ...
    def provider_evidence(self, *_args, cursor="cursor-1"):
        ...
        return {
            "state": "observed", "admission": "accepted", "head_run_id": liveness.head_run_id,
            "head_run_fingerprint": liveness.head_run_fingerprint, "source": "codex-session",
            "source_fingerprint": "a" * 32, "cursor": cursor,
        }
    ...
    def begin(self):
        return continuation_module.begin_red_transition(
            ...
            phase="gate", move_reason="gate is red", verdict_outcome="gate_red",
        )

    def complete(self):
        return continuation_module.complete_red_transition(
            self.runtime, self.task, self.record, self.records, self.payload, "tick-attempt", ref="sample-1",
        )
    ...
    def restart(self):
        return continuation_module._restart_red_worker(
            self.runtime, self.task, self.record, self.records, self.payload, "tick-attempt",
            phase="gate", continuation_reason="session unavailable",
        )
    ...
    def test_delivery_is_saved_before_wake_and_confirmation_before_opening_round(self) -> None:
        self.retain()
        events = []
        self.runtime.save_records.side_effect = lambda *_: events.append(("save", self.record.worker_continuation.stage))
        self.runtime.host.resume_worker.side_effect = lambda *_: events.append(("wake", self.record.worker_continuation.stage))
        result = self.deliver()
    ...
    def test_report_proves_delivery_and_confirmed_replay_never_resends(self) -> None:
        for marker in ("report:done", "report:blocked"):
            with self.subTest(marker=marker):
                ...
                self.pending()
                with mock.patch.object(
                    continuation_module, "_finish_retained_worker_resume", side_effect=OSError("round write died")
    ...
    def test_failed_relaunch_intent_restores_transition_debt(self) -> None:
        self.record.worker_continuation.begin_red_transition("gate", 0, "red", "gate_red", reserved_generation=4)
    ...
    def test_identity_mismatch_blocks_without_rebinding_or_recovering_head(self) -> None:
        ...
        self.runtime.host.provider_progress.return_value = {"state": "identity_mismatch", "reason": "other head"}
    ...
    def test_failed_safe_recovery_intent_save_never_calls_capability(self) -> None:
        ...
        self.assertEqual(self.record.worker_continuation_liveness.recovery_rung, ContinuationRecoveryRung.SAFE_RECOVERY_PENDING)
        self.runtime.host.safe_recover_worker_continuation.assert_not_called()
        self.runtime._stop_worker_confirmed.assert_not_called()
...
class ContinuationOwnershipTests(unittest.TestCase):
    def test_implementation_is_owned_by_the_package_not_runtime_callbacks(self) -> None:
        ...
        runtime = next(n for n in runtime_tree.body if isinstance(n, ast.ClassDef) and n.name == "DispatcherRuntime")
        module_tree = ast.parse(inspect.getsource(continuation_module))
        helpers = {
            "_begin_red_transition", "_complete_red_transition", "_deliver_red_continuation",
            "_observe_retained_continuation_progress", "_block_unadmitted_continuation_liveness",
            "_continuation_recovery_window", "_advance_no_progress_continuation",
            "_finish_retained_worker_resume", "_restart_red_worker", "_record_worker_continuation",
        }
        runtime_names = {n.name for n in runtime.body if isinstance(n, ast.FunctionDef)}
        self.assertFalse(helpers & runtime_names)
        callbacks = {
            n.attr for n in ast.walk(module_tree)
            if isinstance(n, ast.Attribute) and isinstance(n.value, ast.Name) and n.value.id == "runtime"
        }
        self.assertFalse(helpers & callbacks)
        for helper in ("_retained_worker_busy_deferred", "_retained_worker_recovery_window", "_continuation_no_progress_evidence"):
            self.assertNotIn(f"def {helper}(", runtime_source)
        owned = {n.name for n in module_tree.body if isinstance(n, ast.FunctionDef)}
        self.assertTrue({"begin_red_transition", "complete_red_transition", "recover_worker_continuation"} <= owned)
        self.assertNotIn("secretary.dispatcher", inspect.getsource(continuation_module))
        advance = next(n for n in runtime.body if isinstance(n, ast.FunctionDef) and n.name == "_advance_worker")
        advance_source = ast.get_source_segment(runtime_source, advance)
        names = ["_complete_red_transition(", "_worker_report_marker(", "_recover_worker_continuation(", "_handle_worker_report(", "self._wait_watchdog("]
